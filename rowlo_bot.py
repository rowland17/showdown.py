...
#			await room_obj.leave()
	...
	async def on_player(self, params):
		...
		if params[0] == 'p1' and params[1] == username:
			logger.info("myTeam is true")
			myTeam = True
		else:
			logger.info("myTeam is false")
			myTeam = False

	async def on_poke(self, params):
		global p1_team
		global p2_team
		logger.info("ON_POKE RAN")
	...
	async def on_turn(self, room_obj, params):
		...
		if rand == 0:
			...
			logger.info("BAD RAND NUMBER - TURN")
	...
	async def on_request(self, room_id, params):
		logger.info("ON REQUEST - Input type: REQUEST Params: " + str(params))



	#async def on_receive(self, room_id, inp_type, params):
		#logger.info("Input type: " + str(inp_type) + " Params: " + str(params))
